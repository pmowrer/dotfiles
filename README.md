...
In both cases, package installation is handled by Homebrew, and required tools are verified afterwards. The installer also evaluates `brew shellenv` from standard macOS/Linux brew locations so `brew` is usable immediately in non-login shells.
